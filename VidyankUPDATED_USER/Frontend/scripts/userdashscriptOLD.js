...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const navButtons = [toggleButtonDash, toggleButton, toggleButtonHistory, profileButton, notificationButton, helpButton, settingsButton];
    const sections = [dashboardSection, specificTestSection, testResultSection, profileSection, notificationSection, helpSection, settingsSection];
        ...
        Tserieses: {
            FMGE: {
                testseries1: {
                    questionPaper1: [
                        {
                            question: "If <i>n</i> is a natural number, then (6n<sup>2</sup> + 6n) is always divisible by:",
                            options: [
                            ...
                            explanation: "(6n<sup>2</sup> + 6n) = 6n(n + 1), which is always divisible by 6 and 12 both, since n(n + 1) is always even."
                        },
                        {
                            ...
                            explanation: "9548 + 7314 = 16862, and 16862 - 8362 = 8500, which gives the correct answer as 8500."
                        },
                        {
    ...
    const loadQuestions = (category, testSeries, paper) => {
        ...
        if (questions.length > 0) {
                        ...
                        <div class="problem">
                            ...
                            <div class="options">
                                ${questionData.options.map((option, i) => `
                                    <div class="option">
                                        <input class="form-check-input ${getOptionClass(option, questionData.correctAnswer, i)}" type="radio" name="q${index + 1}" id="q${index + 1}${i}" disabled>
                                        <label for="q${index + 1}${i}" class="${getOptionClass(option, questionData.correctAnswer, i)}">${option.text}</label>
                                    </div>
                                `).join('')}
                            </div>
            ...
            questionContainer.innerHTML = '<p>No questions available for this test series.</p>';
        }
    };
    ...
    function updateTime() {
        ...
        currentDateElement.textContent = `Date: ${year}-${month.toString().padStart(2, '0')}-${day.toString().padStart(2, '0')}`;
    }
    ...
    sendOtpBtn.addEventListener("click", function () {
        ...
        document.getElementById("otp-for").textContent = `Enter OTP for ${field}: ${value}`;
    });
    ...
    document.querySelector('.profile-edit').addEventListener('click', function (event) {
    ...
    const resultNotificationsToggle = document.getElementById('result-notifications');
    const testSeriesUpdatesToggle = document.getElementById('test-series-updates');
    const announcementNotificationsToggle = document.getElementById('announcement-notifications');
    ...
    resultNotificationsToggle.addEventListener('change', (e) => {
        const isEnabled = e.target.checked;
        console.log(`Result notifications are ${isEnabled ? 'enabled' : 'disabled'}`);
    });

    testSeriesUpdatesToggle.addEventListener('change', (e) => {
        const isEnabled = e.target.checked;
        console.log(`Test series updates are ${isEnabled ? 'enabled' : 'disabled'}`);
    });

    announcementNotificationsToggle.addEventListener('change', (e) => {
        const isEnabled = e.target.checked;
        console.log(`Announcement notifications are ${isEnabled ? 'enabled' : 'disabled'}`);
    });
